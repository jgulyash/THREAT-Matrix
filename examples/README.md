...
2. **Filters are minimal but representative.** Phase filtering (Threat Lifecycle stage) and actor-profile filtering are the two most common access patterns. Other filters — by matrix, by Cyber-Physical Nexus, by tactic family, by indicator category, by countermeasure domain — follow the same shape.
