...
Escalation scoring now covers **all 190 People-matrix indicator classes across all four phases** — every indicator carries a `temporal_signature`, four `escalation_axes`, a computed `escalation_weight`, and a `severity_band`. Blast is authored to method-revealed collateral ceilings (see the `escalation_rubric` in `framework.json`): the six critical indicators are the mass-casualty Force Application behaviors and the method-revealing capability acquisitions. The `informs_axes` annotation layer (a separate, coarser signal about what an indicator reveals) remains an experimental pilot on the phase-1 chunk (TM0101–TM0104, 23 indicators) pending the sealed-blind inter-rater reliability pass on the rest of the matrix.
