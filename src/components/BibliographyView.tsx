  ...
  return (
    <div className="bib-view">
      ...
      <div className="bib-sub">
        {entries.length} sources cited across the framework. Each entry lists the tactics that
        reference it.
      </div>
      {entries.map(([key, entry]) => {
        const citingTactics = bibReverseMap[key] || [];
        return (
          <div key={key} id={`bib-${key}`} className="bib-entry">
                ...
                <div className="bib-entry-cited-list">
                  {citingTactics.map((t) => (
                    <a
                      ...
                      href={`#/person/tactic/${t.id}`}
                      onClick={(e) => {
                        e.preventDefault();
                        navigate(`/person/tactic/${t.id}`);
                      }}
                      title={t.name}
                    >
                      {t.id}
