...
**Contract Hardening** — the first breaking schema bump since 1.1.0 (schema `1.4.0` → `2.0.0`, content `1.5.2` → `1.6.0`, escalation rubric `1.3.0` → `1.4.0`). The release closes the JSON Schema completely (every fixed-key object rejects unknown keys, every field is typed, per-matrix subschemas pin ids), completes the instance-conditioning contract (sixth factor, demotion doctrine, worked cases), migrates cyber-physical convergence from a stored tactic tag to per-indicator behavioral-mode facets with CPN computed over the Detection Mesh, and ships `conditioning_guidance`: an authored, assessment-scoped investigative-tasking seed on all 815 indicators, written to a ratified voice standard and bounded by an explicit division of labor (responders own the scene, response protocols own the response, the instance record owns generic verification, this field owns the pathway behind the act, the case around the actor, and the act after this one). This is the deliberate pre-platform gate: the V2 platform builds on this frozen contract.

### Changed

- **Schema contract hardened (Contract Hardening closure)** (**breaking**). `additionalProperties: false` now closes all seven previously-open object definitions (tactic, indicator, countermeasure, response_protocol, actor_association, actor_profile, bibliography_entry) and every top-level bag (matrices, warden_integration, ai_architecture, instance_conditioning, phase_mappings, escalation_rubric and all nested sub-objects) — a typo'd or externally-injected unknown key is now rejected. Full field typing landed first (actor_profile's 15 previously-undeclared fields; countermeasure `target_identity`; every escalation_rubric / instance_conditioning sub-field) so no live record false-fails. Each matrix is now validated by its own per-matrix subschema pinning the tactic id prefix (TM/TF/TO/TI) and a `matrix` const equal to the containing key, so a person tactic given a TF id or a `matrix: "facility"` field is rejected by schema alone. `required` extended where every record carries the field; `minItems`/`uniqueItems` added to non-empty and set-semantic arrays (no `multipleOf` on floats — IEEE-754 false-fails). Of the 14 architecture-review invalid states, 5 are now rejected by JSON Schema alone (id-prefix, matrix const, two unknown-key cases, one enum); the other 9 (duplicate-id set-uniqueness, referential integrity, raw duplicate keys, band-vs-weight and weight-vs-composite arithmetic) remain validator-only because JSON Schema cannot express set membership or cross-field arithmetic without `$data`. Classification recorded in the methodology working notes.
- **Reference SPA: Behavioral Mode filter + facet display** (replaces the retired CPN affordance). The CPN filter chip, tactic tag, badge, and heat-map "cyber-enabled" strip are removed with the underlying field. In their place: a **Behavioral Mode** filter (a labeled select in the existing filter-bar style, values Physical / Cyber / Cyber-Physical / Human & Social, each with a hover definition and the CPN acronym spelled out) that shows only tactics carrying an indicator of the selected mode; the indicator detail view now renders each behavior's Behavioral Mode, its crossing mechanism where applicable, a human-and-social co-occurrence marker, and a thin-evidence marker on any escalation axis whose `axis_confidence` is `thin` (with a tooltip stating thin is a wider error bar, not a lower score). The data field remains `modality`; the UI label is "Behavioral Mode" (same data-key-vs-display-label split as People/`person`).
- **Modality facets authored matrix-wide; tactic-level `cpn` retired** (**breaking**; V1.6 CPN facet migration). Every one of the 815 indicators now carries `modality` (`physical` / `cyber` / `cyber_physical` / `human_social`) and the `human_social` boolean, both now **required**. Distribution: People 72/96/21/1, Facility 121/59/17/3, Organizations 4/180/56/0, Infrastructure 36/36/45/68 (physical/human_social/cyber/cyber_physical; Infrastructure reflects the S38 IT-side OT-recon ruling retagging IND-I0105-02/-03 to cyber) (the last re-reviewed since its 185 were authored before `human_social` existed as a value: 42 corrections). Authored by parallel agents against `docs/methodology/modality-authoring-rubric.md`, re-reviewed under the domain-based `human_social` definition, then adversarially reviewed (9 corrections). The tactic-level `cpn` / `cpn_id` / `cpn_notes` fields are removed (superseded by the per-indicator facets; CPN is computed over the Detection Mesh, never stored). The 18 unique digital-observability notes from `cpn_notes` were folded into their tactics' `field_notes`; the rest were superseded by the facets. Field-parity allowlist carve-out for the facet fields removed (now universal).
- **`crossing` renamed `modality`; enum gains `human_social`** (**breaking**; V1.6 modality rulings, 2026-08-01). The per-indicator facet field returns to the locked S32 vocabulary and its four-value modality model: `physical / cyber / cyber_physical / human_social`, where `human_social` positively names behaviors whose only instrument operates on people or institutions (lawfare, coercion, recruitment) instead of forcing them into a seam value or a meaningful absence. The independent `human_social` boolean stays as the co-occurrence axis; `modality: human_social` now schema-enforces the boolean true (if/then). The instrument-vs-venue test governs the cyber line (platform-as-carriage is not cyber); `crossing_mechanism` keeps its name (it attaches only to actual crossings). Infrastructure's 185 facets renamed in place (byte-safe key replacement, reverse-proof; values untouched); matrix-wide authoring lands in this window. Implementation note added to `docs/methodology/RATIONALE-cpn-modality-and-mesh.md` §7.

### Added
- **Instance-conditioning affordance: template, worksheet, and worked cases** (prompt-series P6). `examples/instance-record-template.json` (a schema-valid, fillable `conditioned_assessment` defaulted to all-`unknown`, held at type), `examples/instance-record-worksheet.md` (a one-page analyst worksheet: the six factors as checkbox questions, the escalate-only rule, the raise-guidance patterns, the Q1 demotion doctrine, the inhibitors-exclusion note), and `examples/worked-cases/` (three fictional end-to-end walk-throughs — held at type, raised one band, raised to critical — exercising the contract's edges). `examples/README.md` documents why the framework and its public SPA hold no case data (the interactive entry surface is deferred to V2, where it ships with the SDKs). `scripts/lint/consumer-smoke.py` now validates the template and cases against `$defs.conditioned_assessment` and enforces the escalate-only invariant.
- **Type-level display binding + consumer conditioned-assessment join** (prompt-series P7). The SPA indicator detail marks the Escalation Profile as **Type-level** with an expandable "Class score, not a case assessment" note naming the six instance factors, the raise-only rule, and links to the worksheet and worked cases. `examples/python_consumer.py` gains a `conditioned_assessment` join (escalate-only MAX of type band and instance raise; no lowering path), exercised by `consumer-smoke`.
- **V2 design-constraint record** (prompt-series P11). `docs/methodology/v2-design-constraints.md` locks the computed-CPN mesh contract (never stored; `compute-cpn.py` reference implementation is a 2.0.x fast-follow), the AI-as-amplifier stance, the structured raise-trace and warning-behavior filter deferrals, and the ingestion/observation schema boundary.
- **Native modality validator check V21 + fixture F15** (S38 review hardening). `validate.py` now natively enforces the modality facet contract (presence + enum of `modality` and `human_social`, the `human_social`-modality boolean invariant, `crossing_mechanism` exactly on `cyber_physical`) so the invariant stays gated even where a pipeline skips jsonschema; F15 exercises it. Stale stored-CPN references realigned to computed-CPN doctrine (README, IDENTIFIERS, Infrastructure matrix `scope` text, field-parity allowlist).

- **`conditioning_guidance` replaces the `instance_conditioning_hints` pilot** (**breaking**; the reconceived P8). The informs-keyed pilot (23 People indicators) measured which informs_axes lift at instance time — a rater-judgment signal that proved underivable from informs_axes (5% value match in the derivation stage-1 test) and not convertible to the new concept; the 23 pilot values are retired. In its place, a first-class authored per-indicator field: `conditioning_guidance { probe_factors[], guidance }` — which instance factors are the hidden drivers of danger for this behavior (machine-filterable enum tags) plus a natural-language practitioner directive (max 420 chars, em/en-dash-free by schema pattern). The field is **assessment-scoped**: immediate response actions (alerting, notification, mitigation) are carried by the tactic's `response_protocols`, never by this field — for a harmful act underway the entry verifies the reporting, gauges scope and trajectory, and holds open the actor questions the act does not settle. The deferral is surfaced by a standing caption on every indicator's Conditioning Guidance section in the reference SPA (linking to the tactic's Response Protocols), the schema description, and the methodology docs (field note + ratified authoring rubric). A framework-provided default starting point, NOT prescriptive: consumers may follow, extend, or override it, and its intended consumption is composition across combinations of observed indicators (Detection Mesh), not single-indicator reading. New validator check **V22** (shape, factor enum, uniqueness, length, dash ban) + fixture F16. `instance_conditioning.conditioning_hints` renamed `conditioning_guidance` with status text updated.
- **Remaining typed-but-open schema bags sealed** (S38 review ruling). `additionalProperties: false` added to `detection_mesh` (and its axes items + schema_links), `assessment_guidance` and its five sub-objects, `escalation_axes`, and `informs_axes` — completing the typo-rejection contract across every fixed-key object. `informs_axes` schema description updated to the actual 815/four-matrix coverage (was stale at 190/People-only).

- **axis_confidence on all 815 indicators** (Ruling 10-A, 2026-07-28 session; prompt-series P9). Optional per-axis evidence-basis marker, categorical only (`established / inferred / thin`), rule-derived by the in-repo `scripts/analysis/derive-axis-confidence.py` (detectability ≤ 2.0 → thin / ≥ 6.0 → established; blast_radius within ±0.5 of a documented anchor or tier bound → established; impact and recoverability inferred framework-wide, the honest default). Distribution: detectability 143 established / 625 inferred / 47 thin; blast 761 established / 54 inferred. New `escalation_rubric.axis_confidence_rubric` records the levels, rules, and the misread guard (confidence is evidence basis, NOT severity — never an automatic demotion). New validator check **V20** (axis names + enum values). Insertion-only splice with reverse-edit byte proof.
...
- **`conditioning_effect` dropped from `conditioned_assessment`** (**breaking**; V1.6 dogfood finding). The held-vs-raised distinction is a pure function of `conditioned_priority` vs `type_severity_band` (raised iff priority exceeds band) — the same base-vs-environmental comparison CVSS uses — so storing it duplicated state that could silently drift (the schema validates each enum independently and could not catch an effect/priority contradiction). It is now derived on read, not stored. The escalate-only doctrine is unchanged and remains stated in `conditioned_priority`'s description and `instance_conditioning.escalate_only_rule`.
...
- **Weight review — fixation re-tiered** (follow-on to Ruling 7). IND-O0101-02 (grievance narrowing to fixation on a named organization) was authored in the earliest-diffuse impact tier though it presages a targeted campaign; `impact_potential` 3.0 → 4.5 (presages tier), weight 2.78 → 3.08, band low → medium. Its warning value was already carried by `informs_axes` (intent/target strong); the fix was the impact tier only. IND-O0101-04 (pre-commitment identifier collection) confirmed correctly low (ambient pre-operational recon). Band distribution 14 low / 432 medium / 282 high / 87 critical.
- **Bottom-cut recalibration — low band restored** (Ruling 7). The medium threshold moves 2.5 → 3.0 against the realized post-reconciliation distribution; the low band, which had silently held zero of 815 indicators, now holds 15 investigative/attributional trace behaviors (egress, cleanup, attribution, ambient pre-operational recon). High and critical thresholds unchanged. Band distribution now 15 low / 431 medium / 282 high / 87 critical. `escalation_weight` values untouched (bands only). `scripts/lint/validate.py` now reads `severity_thresholds` from the data (no hardcoded cut) and emits a non-failing `W-BAND` warning for any zero-population band — a regression guard against the next silent dead band. Two org target-development warning behaviors (IND-O0101-02, IND-O0101-04) band low at this cut and are flagged for a targeted weight review. New methodology note `docs/methodology/bottom-cut-recalibration.md`.
- **Severity-floor clarification — weapon-threat custody** (Ruling 8-A). `severity_floor_rule.applies_to` extended: taking or maintaining custody or control of a person by weapon threat during in-progress execution (weapon displayed at the victim, control being established) counts as taking physical custody, with a scope guard excluding brandishing or communicated threats outside a seizure. Applied to IND-0303-04 (band medium → critical; `escalation_weight` 4.86 unchanged, per the floor rule). Floored count 42 → 43. New methodology note `docs/methodology/severity-floor-clarifications.md`.
- **Aftermath seam reconciliation** (`escalation_rubric` `rubric_version` 1.2.0 → 1.3.0; rulings 3, 2-B, 4-A, 5-A, 6-A). Phase-4 `blast_radius_potential` and `impact_potential` now encode only the scope an aftermath behavior presages or enables going forward, not the completed attack's scope. 165 of 215 aftermath indicators re-authored to the cross-matrix trace floor; 66 moved high → medium (framework distribution 381/348/86 → 447/282/86; critical unchanged). The Facility mass-casualty blast floor is scoped to Phases 1-3 and forward-looking Phase 4 (promoted into the rubric as `blast_radius_guidance.aftermath_rule`); the Organizations continuing-harm classes and a narrow instructional/celebratory copycat-enablement tier stay elevated by design, with an anti-harmonization guard recorded so the cross-matrix asymmetry is preserved. Two `correlates_with` edges added for continuation-threat claims (Ruling 5-A). New methodology note `docs/methodology/aftermath-seam-reconciliation.md`. Weights and bands recomputed together; no schema change.
