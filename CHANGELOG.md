...
- **`conditioning_guidance` replaces the `instance_conditioning_hints` pilot** (**breaking**; the reconceived P8). The informs-keyed pilot (23 People indicators) measured which informs_axes lift at instance time — a rater-judgment signal that proved underivable from informs_axes (5% value match in the derivation stage-1 test) and not convertible to the new concept; the 23 pilot values are retired. In its place, a first-class authored per-indicator field: `conditioning_guidance { probe_factors[], guidance }` — which instance factors are the hidden drivers of danger for this behavior (machine-filterable enum tags) plus a natural-language practitioner directive (max 420 chars, em/en-dash-free by schema pattern). A framework-provided default starting point, NOT prescriptive: consumers may follow, extend, or override it, and its intended consumption is composition across combinations of observed indicators (Detection Mesh), not single-indicator reading. New validator check **V22** (shape, factor enum, uniqueness, length, dash ban) + fixture F16. `instance_conditioning.conditioning_hints` renamed `conditioning_guidance` with status text updated.
