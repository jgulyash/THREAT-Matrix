...
- **Modality facets authored matrix-wide; tactic-level `cpn` retired** (**breaking**; V1.6 CPN facet migration). Every one of the 815 indicators now carries `modality` (`physical` / `cyber` / `cyber_physical` / `human_social`) and the `human_social` boolean, both now **required**. Distribution: People 72/96/21/1, Facility 121/59/17/3, Organizations 4/180/56/0, Infrastructure 36/43/70/36 (the last re-reviewed since its 185 were authored before `human_social` existed as a value: 42 corrections). Authored by parallel agents against `docs/methodology/modality-authoring-rubric.md`, re-reviewed under the domain-based `human_social` definition, then adversarially reviewed (9 corrections). The tactic-level `cpn` / `cpn_id` / `cpn_notes` fields are removed (superseded by the per-indicator facets; CPN is computed over the Detection Mesh, never stored). The 18 unique digital-observability notes from `cpn_notes` were folded into their tactics' `field_notes`; the rest were superseded by the facets. Field-parity allowlist carve-out for the facet fields removed (now universal).
