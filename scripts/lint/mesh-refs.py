...
AND matches the canonical IND-XXXX-XX / CM-XXXX-XX / RP-XXXX-XX format.
...
IND_PATTERN = re.compile(r"^IND-[0-9]{4}-[0-9]{2}$")
CM_PATTERN = re.compile(r"^CM-[0-9]{4}-[0-9]{2}$")
RP_PATTERN = re.compile(r"^RP-[0-9]{4}-[0-9]{2}$")
        ...
        for tactic in matrix_val.get("tactics", []) or []:
                ...
                for ref in ind.get("correlates_with", []) or []:
                    if not IND_PATTERN.match(ref):
                        errors.append(
                            ...
                            f"'{ref}' does not match IND-XXXX-XX format"
                        )
            ...
            for cm in tactic.get("countermeasures", []) or []:
                ...
                for ref in cm.get("compensates_for", []) or []:
                    if not CM_PATTERN.match(ref):
                        errors.append(
                            f"[{matrix_key}/{tactic_id}/{cm_id}] compensates_for: "
                            f"'{ref}' does not match CM-XXXX-XX format"
                        )
                    elif ref not in cms:
                        errors.append(
                            f"[{matrix_key}/{tactic_id}/{cm_id}] compensates_for: "
                ...
                for ref in rp.get("coordinates_with", []) or []:
                    if not RP_PATTERN.match(ref):
                        errors.append(
                            ...
                            f"'{ref}' does not match RP-XXXX-XX format"
                        )
                    elif ref not in rps:
                        errors.append(
                            f"[{matrix_key}/{tactic_id}/{rp_id}] coordinates_with: "
